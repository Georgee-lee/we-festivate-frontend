...
class Post extends React.Component {
  ...
  //   try {
  //     const res = await fetch(`http://10.58.2.168:8000/event/detail/${this.props.postId}`);
          ...
          <PostWrap>
            ...
            <PostHeader>
              ...
              <TitleBox>
                <TitleHead>
                  ...
                  <p style={{ color: 'gray', fontSize: 20, marginTop: 5 }}>at {post.building} &nbsp; {post.place}</p>
                  <br /><br /><br />
                  <p style={{ fontWeight: 'bold', marginBottom:20 }}>일시</p>
                  <p style={{ margin: '0 0 5px 0' }}>{post.date}</p> 
                  <p style={{ margin: 0 }}>{post.start_time} ~ {post.end_time}</p>
                </TitleHead>
              </TitleBox>
            </PostHeader>
            ...
            <InfoBox>
              <span style={{ display: 'inline-block', padding: 15 }}>
                <FontAwesomeIcon icon={faUsers} style={{ width: '1em', height: '1em' }}/>&nbsp;&nbsp; {post.current_rsvp}명 /&nbsp; { post.max_rsvp > 0 ? post.max_rsvp+'명' : '제한없음' }
              </span>
            <JoinBtn>
              RSVP
            </JoinBtn>
            ...
            {/* 본문이 위치한 곳 */}
            <div style={{ whiteSpace: 'pre' }}>{post.main_text}</div>
            {/* 지도가 위치한 곳 */}
